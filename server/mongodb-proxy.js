...
const dbs = config.get("dbs")

// Called by test
app.all("/", function (req, res, next) {
  logRequest(req.body, "/");
  setCORSHeaders(res);
  const dbConfig = dbs[req.body.db]
...
app.all("/search", function (req, res, next) {
  ...
  if (queryArgs.err != null) {
    ...
    doTemplateQuery(requestId, queryArgs, dbs[req.body.db], res, next);
  }
});
...
// { _id : <id> }
function doTemplateQuery(requestId, queryArgs, dbConfig, res, next) {
  if (queryArgs.err == null) {
    // Database Name
